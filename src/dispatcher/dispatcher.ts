...
import { DecisionSchema, DISPATCH_JSON_SCHEMA } from "./schema.ts";
...
export class Dispatcher {
  ...
  private async process(id: string) {
    ...
    for (const effort of [this.cfg.dispatcher.effort, this.cfg.dispatcher.retry_effort]) {
      ...
      if (last.decision && last.decision.confidence === "high") break;
    ...
    if (!last.decision || last.decision.confidence === "low") {
  ...
  private async decide(text: string, ctx: string, effort: string, prevError: string) {
    ...
    const args = ["-p", "--output-format", "json", "--json-schema", JSON.stringify(DISPATCH_JSON_SCHEMA), "--max-turns", "1", "--tools", "", "--no-session-persistence",
      "--model", this.cfg.dispatcher.model, "--effort", effort, "--append-system-prompt", DISPATCH_SYSTEM_PROMPT, prompt];
    const t0 = now();
