 ...
 *  `mode` drops what exists only to route: a project's path and keywords (both there to match a message to a
 *  worktree) and the decision annotations on the chat lines. Answering a question needs the rest — which projects
 *  exist, what is running and how it is going — so Ask carries it too, just without the routing apparatus. */
export function buildContext(db: Database, mode: "route" | "ask" = "route"): string {
  ...
  return [
    "[projects]", ...(projects.length ? projects.map((p) => `- ${p.name}${route ? ` (${p.path})` : ""} — ${p.description}${route ? `; keywords: ${p.keywords.join(", ")}` : ""}`) : ["- (none registered)"]),
    ...
    "[recent chat]", ...recent.map((m) => `- ${m.role}: ${m.text.slice(0, 200)}${route && m.dispatch_json ? ` → ${m.dispatch_json.action}${ids(m.dispatch_json)}` : ""}`),
  ].join("\n");
}
