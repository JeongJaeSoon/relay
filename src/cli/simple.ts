...
export async function send(rest: string[]) { const text = rest.filter((x, i) => !(x === "--to" || rest[i - 1] === "--to")).join(" ").trim(); if (!text) throw new CliError('usage: relay send "<message>" [--to <task>]', 2);
  const r = await client().post("/messages", { text, client_message_id: crypto.randomUUID(), source: "cli", ...(arg(rest, "--to") ? { reply_to_task_id: await resolveTask(arg(rest, "--to")!) } : {}) }); process.stdout.write(`Accepted ${r.message_id} · ${text.length > 40 ? text.slice(0, 40) + "…" : text}\n`); }   // echoes the body: with `relay "<msg>"` the shell decides the split, and you want to see it
export async function resolveTask(idOrDisplay: string): Promise<string> { if (/^[0-9a-f-]{36}$/.test(idOrDisplay)) return idOrDisplay; const s = await client().get("/tasks?include=closed"); const t = s.tasks.find((x: any) => x.display_id === idOrDisplay.toUpperCase() || x.uuid.startsWith(idOrDisplay)); if (!t) throw new CliError(`relay: task not found: ${idOrDisplay}`); return t.uuid; }
export async function ls(rest: string[]) { const s = await client().get(has(rest, "--all") ? "/tasks?include=closed" : "/tasks"); if (has(rest, "--json")) { process.stdout.write(JSON.stringify(s.tasks, null, 2) + "\n"); return; }
  ...
  const head = ["ID", "Status", "Project", "Title", "Elapsed", "Session"]; const w = head.map((h, i) => Math.max(width(h), ...rows.map((r) => width(String(r[i])))));
  for (const r of [head, ...rows]) process.stdout.write(r.map((c, i) => pad(String(c), w[i])).join("  ") + "\n"); if (!rows.length) process.stdout.write("(no tasks)\n"); }
export async function tail(rest: string[]) { const uuid = await resolveTask(rest[0] ?? ""); const c = client(); const snap = await c.get("/tasks?include=closed"); process.stdout.write(`tail ${uuid} (Ctrl-C to stop)\n`);
  await new Promise<void>((done) => c.ws(snap.as_of_seq, (f) => {                                     // from as_of_seq: only what happens from now on, never the whole history
    if (f.type === "task.event" && f.task_uuid === uuid) process.stdout.write(`${new Date(f.event.occurred_at).toLocaleTimeString("en-GB", { hour12: false })}  ${f.event.type}  ${JSON.stringify(f.event.payload).slice(0, 160)}\n`);
    if (f.type === "task.updated" && f.task.uuid === uuid) process.stdout.write(`  → ${f.task.status}${f.task.last_step ? " · " + f.task.last_step : ""}\n`);
  }, () => { console.error("relay: connection closed"); done(); })); }
export async function open() { const c = client(); Bun.spawn(["open", `${c.base}/`]); process.stdout.write(`${c.base}/\n`); }
export async function pause() { await client().post("/pause"); process.stdout.write("kill switch ON — new dispatches stopped, running workers asked to stop\n"); }
export async function resumeAll() { await client().post("/resume-all"); process.stdout.write("kill switch OFF — resumed\n"); }
