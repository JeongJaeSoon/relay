...
export async function send(rest: string[]) { const text = rest.filter((x, i) => !(x === "--to" || rest[i - 1] === "--to")).join(" ").trim(); if (!text) throw new CliError('usage: relay send "<message>" [--to <task>]', 2);
  const r = await client().post("/messages", { text, client_message_id: crypto.randomUUID(), source: "cli", ...(arg(rest, "--to") ? { reply_to_task_id: await resolveTask(arg(rest, "--to")!) } : {}) }); process.stdout.write(`Accepted ${r.message_id}\n`); }
