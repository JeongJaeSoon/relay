...
export const MIGRATIONS: string[] = [
  ...
  create table projects(id text primary key, name text unique not null, path text unique not null, description text not null default '',
    keywords_json text not null default '[]', base_ref text not null default 'fresh', is_git integer not null default 1, created_at integer not null);
  create table tasks(
    ...
    size text not null check(size in ('small','normal','epic')), effort text not null, model text not null,
    ...
    process_generation integer not null default 0, turn_state text not null default 'idle', attach_state text not null default 'none', attached_by text,
  ...
  create table messages(
    id text primary key, role text not null, source text not null default 'user', client_message_id text unique,
    dispatch_state text not null default 'pending', text text not null, task_uuid text references tasks(uuid), reply_to_task_uuid text,
  ...
  create table commands(id text primary key, task_uuid text not null references tasks(uuid), kind text not null, payload_json text not null,
