...
export class TaskService {
  ...
  close(taskUuid: string) {
    ...
    // A queued task has to leave the queue before anything pumps, or the scheduler grants a slot to the task being
    // closed — the same exit `interrupt` gives it. The rm turns it into `closed` when it lands.
    if (t.status === "queued") this.status(t, "cancelled", { ended_at: now(), qhead: false });
