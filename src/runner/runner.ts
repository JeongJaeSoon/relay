 ...
 *  a "not pushed anywhere" refusal does not have. `retryable` separates the one refusal that resolves without anybody
 *  doing anything (the worktree is locked because the session is still exiting) from the two that need a person. */
export interface RmOutcome { worktreeKept: boolean; reason?: string; keptPath?: string; retryable?: boolean }
