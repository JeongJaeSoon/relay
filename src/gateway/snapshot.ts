...
export function snapshot(db: Database, cfg: Config, includeClosed = false): TasksSnapshot {
  const as_of_seq = (db.query("select coalesce(max(seq),0) s from ws_frames").get() as any).s;   // the frame cursor, not the event cursor: a client only advances on frames it applies
