...
export function buildMcpServer(c: C) {
  ...
  s.registerTool("relay_send", { description: "Send a message to the relay orchestrator (a new task, a follow-up, or an answer to a question). Pass a task id like T-08 in `to` to deliver it straight to that task.", inputSchema: { text: z.string().min(1), to: z.string().optional() } },
    guard(async ({ text: t, to }) => { const reply = await resolve(to); if (to && !reply) return text(`Task not found: ${to}`); const r = await c.post("/messages", { text: t, client_message_id: crypto.randomUUID(), source: "mcp", ...(reply ? { reply_to_task_id: reply } : {}) }); return text(`Accepted ${r.message_id}${reply ? ` → ${to}` : ""}`); }));
