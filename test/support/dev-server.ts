// test/support/dev-server.ts — UI development harness: real server, fake sessions.
...
// Keep synthetic sessions and config separate from an installed Relay instance.
const sandbox = mkdtempSync(join(tmpdir(), "relay-dev-"));
const project = join(sandbox, "myapp");
process.env.RELAY_HOME = join(sandbox, "home");
process.env.RELAY_LOG_DIR = join(sandbox, "logs");
process.env.RELAY_NO_FILE_LOG = "1";
mkdirSync(process.env.RELAY_HOME, { recursive: true });
writeFileSync(join(process.env.RELAY_HOME, "config.toml"), `port = ${Number(process.env.RELAY_DEV_PORT || 8814)}\n`);
...
const seq = async (short: string, task: string, prompt: string, gen: number) => {
  ...
  const tools = [["Read", { file_path: join(project, "src/auth.ts") }], ["Edit", { file_path: join(project, "src/auth.ts") }], ["Bash", { command: "bun test" }]] as const;
...
if (!(ctx.db.query("select 1 from projects where name='myapp'").get())) await fetch(`http://127.0.0.1:${ctx.cfg.port}/api/projects`, { method: "POST", headers: { authorization: `Bearer ${ctx.tokens.api}`, "content-type": "application/json" }, body: JSON.stringify({ name: "myapp", path: project, description: "fake app", keywords: ["auth"] }) });
