...
export async function buildTestApp(runClaude?: RunClaude, max = 10) {
  ...
  let hub!: WsHub; const log = new EventLog(db, (f) => hub.broadcast(f), cfg); hub = new WsHub(() => log, cfg, db);
  ...
  const ctx: AppContext = { db, cfg, log, hub, tokens: { api: "API", hook: "HOOK" }, services: { ingestDeps: svc.ingestDeps, tasks: svc, outbox, scheduler, dispatcher, permits, pendingPermissions }, dashboardHtml: async () => "<html><head></head><body></body></html>" };
  ...
  return { app, ctx, db, log, hub, runner, svc, dispatcher, outbox, permits, scheduler, req, hookReq, seedTask, settle, invariants, setMax: (n: number) => { maxAgents = n; } };
}
