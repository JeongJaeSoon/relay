...
function setup(run: RunClaude) {
  ...
  const msg = (text: string, taskUuid: string | null = null, ask = false) => { const id = ulid(); log.emit({ type: "message.received", payload: { id, role: "user", source: "user", client_message_id: id, dispatch_state: "pending", text, task_uuid: taskUuid, reply_to_task_uuid: null, ask, dispatch_json: null, dispatch_error: null, created_at: 1 } }); return id; };
...
describe("Ask mode", () => {
  test("a question is answered, never dispatched — the model's action is ignored", async () => {
    ...
    const id = s.msg("why did T-02 fail", null, true); s.d.enqueue(id); await settle();
  ...
  test("a model that answers with a routing decision and no answer is a failed call, not a task", async () => {
    ...
    const id = s.msg("refactor auth in myapp", null, true); s.d.enqueue(id); await settle();
  ...
  test("the ask prompt drops the routing apparatus but keeps what answering needs, and the question reaches the model as typed", async () => {
    ...
    expect(prompt.endsWith("[user message]\nwhy did T-02 fail")).toBe(true);                            // nothing but the question
  ...
  test("the reviewer's repro: a question the fast path cannot answer still reaches the model with the failing task in it", async () => {
    ...
    const id = s.msg("T-02가 왜 실패했어", null, true); s.d.enqueue(id); await settle();
  ...
  test("a secret a worker printed never reaches the argv of the one-shot", async () => {
    ...
    const id = s.msg("what did it print", "u1", true); s.d.enqueue(id); await settle();
  ...
  test("an ask call is smaller than the routing call for the same words", async () => {
    ...
    s.d.enqueue(s.msg("why did T-02 fail", null, true)); await settle();
  ...
  test("a status question in Ask mode still takes the fast path — no LLM call", async () => {
    ...
    const id = s.msg("상태?", null, true); s.d.enqueue(id); await settle();
  ...
  test("a task-scoped question reads that task's state and transcript, and never touches the worker", async () => {
    ...
    const id = s.msg("why is T-02 stuck", "u1", true); s.d.enqueue(id); await settle();
  ...
  test("a task-scoped question is never the system status fast path", async () => {
    ...
    const id = s.msg("status?", "u1", true); s.d.enqueue(id); await settle();
