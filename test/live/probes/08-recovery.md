...
1. `bun test/live/probes/sandbox.ts`
2. Start the receiver on 8799 with the spool in front of it, i.e. inject `--settings` whose hooks are
   `command` hooks running `bun test/live/probes/hook-spool.ts <Event> <spoolDir> http://127.0.0.1:8799/hook`
   (so a POST that fails leaves a file behind), and start the receiver:
   `bun test/live/probes/hookd.ts 8799 test/artifacts/live/08-recovery.jsonl`
...
5. `claude agents --json --all > test/artifacts/live/agents-json-recovery.json`, count the spool files,
   then try to re-control each worker (`claude stop` / `claude --bg --resume <uuid> "…"`).
6. Record the completed table and raw probe output under `test/artifacts/live/`.
