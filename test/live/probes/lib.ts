// test/live/probes/lib.ts — shared helpers for Phase 0 spike scripts.
import { appendFileSync, existsSync, mkdirSync, readdirSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { cleanupScopedProbes, stopAndRemove } from "../../helpers/probe-cleanup.ts";

if (process.env.RELAY_LIVE_TESTS !== "1") throw new Error("Live probes require RELAY_LIVE_TESTS=1; see test/README.md");
export const SPIKES = new URL("../../artifacts/live/", import.meta.url).pathname;
export const RESULTS = join(SPIKES, "results");
export const FIXTURES = join(SPIKES, "captures");
...
export async function agents(all = false): Promise<any[]> {
  ...
  if (r.code !== 0) throw new Error("Claude roster query failed; cleanup status is unknown");
  const rows = JSON.parse(r.stdout);
  if (!Array.isArray(rows)) throw new Error("Claude roster is not an array");
  return rows;
}
export const spikeAgents = async (all = false) => (await agents(all)).filter((a) => String(a.name ?? "").startsWith("relay-spike:") && (a.cwd === SANDBOX || String(a.cwd ?? "").startsWith(SANDBOX + "/")));
...
// never register them just to watch. See capabilities.worktreeCreateHook and test/fixtures/worktree-create.json.
...
/** Build the measured cross-session frame (test/fixtures/peer-frames.json). */
...
export function spawnHookd(port: number, log: string, args: string[] = [], env: Record<string, string> = {}) {
  const p = Bun.spawn(["bun", join(import.meta.dir, "hookd.ts"), String(port), log, ...args], { stdout: "inherit", stderr: "inherit", env: { ...CLEAN_ENV, ...env } });
  HOOKDS.push(p);
  return p;
}
...
export async function stopAndRm(short: string) {
  await stopAndRemove({ list: () => agents(true), stop: async (id) => { const r = await sh(["claude", "stop", id], { timeoutMs: 15_000 }); if (r.code !== 0) throw new Error(`Stop failed: ${id}`); }, rm: async (id) => { const r = await sh(["claude", "rm", id], { timeoutMs: 15_000 }); if (r.code !== 0) throw new Error(`Removal refused: ${id}`); } }, SANDBOX, short);
}
/** Stop+rm every relay-spike:* session (backstop so a failed run never leaks subscription usage). */
export async function cleanupSpikes() { await cleanupScopedProbes({ list: () => agents(true), stop: async (id) => { const r = await sh(["claude", "stop", id], { timeoutMs: 15_000 }); if (r.code !== 0) throw new Error(`Stop failed: ${id}`); }, rm: async (id) => { const r = await sh(["claude", "rm", id], { timeoutMs: 15_000 }); if (r.code !== 0) throw new Error(`Removal refused: ${id}`); } }, SANDBOX); }
...
let cleaning = false;
const bail = (code: number) => async (e?: unknown) => { if (cleaning) return; cleaning = true; if (e) console.error(e); try { await cleanupSpikes(); } catch (err) { console.error("Probe cleanup incomplete:", err); code = 1; } finally { killHookds(); } process.exit(code); };
