// test/live/probes/02-peer.ts — register this process as a Claude Code messaging peer and log/send raw socket frames.
// usage: bun test/live/probes/02-peer.ts            (registers as "relay-spike" and listens; type JSON lines on stdin to send)
//   stdin: {"to":"/tmp/cc-socks/<pid>.sock","text":"hello"}      → sends a frame built from test/fixtures/peer-frames.json (or a guess) and logs replies
