...
function environment(width: number) {
  ...
  const document: any = { activeElement: null, body: {}, getElementById: (id: string) => nodes.get(id) };
  function node(id: string, root?: string) {
    ...
    const n: any = { id, dataset: {}, tagName: "BUTTON", textContent: id, isConnected: true, inert: false, visible: true, children: [],
      ...
      closest: (selector: string) => selector === "[inert]" ? (n.inert || (root && nodes.get(root).inert) ? n : null) : selector === "details" ? null : root ? nodes.get(root) : null,
