...
test("message day keys are derived from each persisted timestamp, never the render time", () => {
  const first = new Date(2026, 8, 6, 23, 59).getTime(); const next = new Date(2026, 8, 7, 0, 1).getTime();
  expect(messageDayKey(first)).not.toBe(messageDayKey(next));
