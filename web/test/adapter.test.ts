...
test("the dispatcher's decision is not drawn twice: the badge-chip row wins over the server's system text row", () => {
  ...
  expect(isDispatcherBadgeRow(sys("▶ [myapp] auth 리팩토링 시작 (T-01)"))).toBe(false);                       // the started row stays
  expect(isDispatcherBadgeRow(sys("T-01 auth을(를) 종료할까요? [종료 확인: POST /api/tasks/u1/close]"))).toBe(false);   // so does the close prompt
