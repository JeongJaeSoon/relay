...
test("vertical, horizontal and diagonal wheel gestures pan without changing zoom",()=>{
  const h=setup();h.send({deltaY:25});expect(h.view).toEqual({x:50,y:55,k:2});
  h.send({deltaY:-25});
  h.send({deltaX:40});expect(h.view).toEqual({x:10,y:80,k:2});
  h.send({deltaX:-10,deltaY:12});expect(h.view).toEqual({x:20,y:68,k:2});expect(h.counts()).toEqual([4,4]);
});
...
test("only Ctrl or Meta modified wheel gestures retain cursor-centered zoom",()=>{
